...
int main()
{
    ...
    try
    {
        ...
        if (event.command.get_command_name() == "setpayment")
        {
            ...
            event.reply(dpp::message("Payment details set for " + (user.username) + " to:\n" + payment_info).set_flags(dpp::m_ephemeral));
        }
        if(event.command.get_command_name() == "getpayment"){
            dpp::snowflake user_id = std::get<dpp::snowflake>(event.get_parameter("user"));
            std::string user_info = Get_info::get_info(user_state, user_id.str()).value_or("No payment details available for user");
		...
		if (dpp::run_once<struct register_bot_commands>()) {
            ...
            setpayment.add_option(dpp::command_option(dpp::co_string, "payment_info", "Your payment details. Don't put anything you don't want other people to see in here!", true));
            dpp::slashcommand getpayment = dpp::slashcommand("getpayment", "Get payment details for another user", bot.me.id);
            getpayment.add_option(dpp::command_option(dpp::co_user, "user", "Whose payment details you want to see", true));
